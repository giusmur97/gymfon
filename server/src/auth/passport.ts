...
type ProfileLike = { id: string; displayName?: string | null; emails?: { value: string }[] };

async function upsertOAuthUser(provider: string, providerAccountId: string, profile: ProfileLike) {
    ...
    passport.use(
      new GoogleStrategy(
        ...
        async (_accessToken, _refreshToken, profile, done) => {
          try {
            const user = await upsertOAuthUser("google", profile.id, profile as any);
            done(null, user);
          } catch (e) {
            done(e as Error);
          }
  ...
  if (process.env.FACEBOOK_CLIENT_ID && process.env.FACEBOOK_CLIENT_SECRET) {
    passport.use(
      new FacebookStrategy(
        ...
        async (_accessToken, _refreshToken, profile, done) => {
          try {
            const user = await upsertOAuthUser("facebook", profile.id, profile as any);
            done(null, user);
          } catch (e) {
            done(e as Error);
          }
  ...
  if (process.env.APPLE_CLIENT_ID && process.env.APPLE_TEAM_ID && process.env.APPLE_KEY_ID && process.env.APPLE_PRIVATE_KEY) {
    passport.use(
      new (AppleStrategy as any)(
        ...
        async (_accessToken: string, _refreshToken: string, idToken: any, _profile: any, done: any) => {
          try {
