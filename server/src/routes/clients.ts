...
import { 
  protectGestionaleRoute, 
  protectAdminRoute, 
  protectStaffRoute,
  protectClientProfileRoute,
  requirePermission,
...
router.post('/add-manual',
  authenticateToken,
  requirePermission('canAddClientsManually'),
  async (req, res) => {
    try {
...
router.post('/:clientId/enable-features',
  authenticateToken,
  requirePermission('canManageAllClients'),
  async (req, res) => {
    try {
      const { clientId } = req.params;
...
router.post('/:clientId/disable-features',
  authenticateToken,
  requirePermission('canManageAllClients'),
  async (req, res) => {
    try {
...
router.get('/:clientId/access-status',
  authenticateToken,
  ...protectClientProfileRoute,
  async (req, res) => {
    try {
...
router.get('/me/permissions',
  ...
  async (req, res) => {
    try {
      ...
      const clientPermissions = (req as any).clientPermissions || {};
      const hasGestionaleAccess = (req as any).hasGestionaleAccess || false;
...
import { 
  clientFullProfileSchema,
  personalInfoSchema,
  sportsAnamnesisSchema,
  physiologicalAnamnesisSchema,
  pathologicalAnamnesisSchema,
  nutritionDiarySchema
  ...
  async (req, res) => {
    try {
      ...
      const updatedClient = await prisma.user.update({
        ...
        data: {
          preferences: {
            ...((existingClient.preferences as any) || {}),
...
router.get('/:clientId/profile',
  ...
  async (req, res) => {
    try {
      ...
      const clientProfile = (client.preferences as any)?.clientProfile || {};
...
router.put('/:clientId/profile/:section',
  authenticateToken,
  ...protectClientProfileRoute,
  async (req, res) => {
    try {
      ...
      const currentPreferences = (existingClient.preferences as any) || {};
...
router.get('/:clientId/measurements',
  authenticateToken,
  ...protectClientProfileRoute,
  async (req, res) => {
    try {
      ...
      const measurements = (client.preferences as any)?.bodyMeasurements || [];
...
router.post('/:clientId/measurements',
  authenticateToken,
  requirePermission('canManageAllClients'),
  async (req, res) => {
    try {
...
router.put('/:clientId/measurements/:measurementId',
  authenticateToken,
  requirePermission('canManageAllClients'),
  async (req, res) => {
    try {
      ...
      const measurementIndex = currentMeasurements.findIndex((m: any) => m.id === measurementId);
...
router.delete('/:clientId/measurements/:measurementId',
  authenticateToken,
  requirePermission('canManageAllClients'),
  async (req, res) => {
    try {
      ...
      const updatedMeasurements = currentMeasurements.filter((m: any) => m.id !== measurementId);
...
router.get('/:clientId/measurements/analytics',
  authenticateToken,
  ...protectClientProfileRoute,
  async (req, res) => {
    try {
      ...
      const calculateTrend = (field: string) => {
        const values = filteredMeasurements
          ...
          .map((m: any) => ({ date: m.date, value: m[field] }));
      ...
      const circumferencesTrend: any = {};
      const circumferenceFields = ['chest', 'waist', 'hips', 'thigh', 'arm', 'forearm', 'neck', 'calf'];
      
      circumferenceFields.forEach(field => {
        const values = filteredMeasurements
          .filter((m: any) => m.circumferences?.[field] != null)
          .map((m: any) => ({ date: m.date, value: m.circumferences[field] }));
