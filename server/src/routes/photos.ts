 ...
 */
router.post('/upload', ...requireAuth(['admin', 'staff']), upload.single('photo'), async (req: express.Request, res: express.Response) => {
  try {
    if (!req.file) {
...
router.get('/:clientId', ...requireAuth(['admin', 'staff', 'client']), async (req: express.Request, res: express.Response) => {
  try {
    ...
    if ((req as any).user?.role === 'client' && (req as any).user?.id !== clientId) {
...
router.get('/:clientId/timeline/:type', ...requireAuth(['admin', 'staff', 'client']), async (req: express.Request, res: express.Response) => {
  res.json({ success: true, photos: [] });
});

router.post('/compare', ...requireAuth(['admin', 'staff', 'client']), async (req: express.Request, res: express.Response) => {
  res.json({ success: true, comparison: null });
});

router.delete('/:photoId', ...requireAuth(['admin', 'staff']), async (req: express.Request, res: express.Response) => {
  res.json({ success: true, message: 'Photo deleted' });
});

router.patch('/:photoId', ...requireAuth(['admin', 'staff']), async (req: express.Request, res: express.Response) => {
  res.json({ success: true, photo: {} });
});

router.get('/upload-progress/:uploadId', ...requireAuth(['admin', 'staff']), async (req: express.Request, res: express.Response) => {
