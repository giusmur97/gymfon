...
import {
  ...
  detailedNutritionDiarySchema,
  generalInfoSchema,
  trainingPreferencesSchema,
  supplementationSchema,
  eligibilitySchema,
  privacyConsentsSchema,
  extendedClientFullProfileSchema,
...
router.get('/:clientId/profile', authenticateToken, async (req, res) => {
  try {
    ...
    const profile = ((client.preferences as any) || {}).clientProfile || {};
...
router.put('/:clientId/profile/:section', authenticateToken, async (req, res) => {
  try {
    const { clientId, section } = req.params as { clientId: string; section: string };
