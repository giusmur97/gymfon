    ...
    if (clientProfile.cloudProvider && clientProfile.cloudFolderId) {
      try {
        ...
        const uploadResult = await cloudStorage.uploadFile(
          ...
          clientProfile.cloudProvider as 'google_drive' | 'dropbox',
          clientProfile.cloudFolderId
        );
    ...
    if (document.cloudProvider && document.cloudFileId) {
      try {
        ...
        await cloudStorage.deleteFile(
          document.cloudFileId,
          document.cloudProvider as 'google_drive' | 'dropbox'
        );
      } catch (cloudError) {
  ...
  try {
    ...
    const validationStatus = {
      ...
      expiredDocuments: documents.filter(d => 
        d.expiryDate && d.expiryDate < new Date() && d.isValid
      ).length,
      expiringDocuments: documents.filter(d => {
        if (!d.expiryDate || !d.isValid) return false;
    ...
    requiredTypes.forEach(type => {
      const hasValidDocument = documents.some(d => 
        d.type === type && 
        d.isValid && 
