...
export class DropboxService {
   ...
   */
  async uploadFile(options: DropboxUploadOptions): Promise<DropboxFile> {
    try {
   ...
   */
  async getFileInfo(filePath: string): Promise<DropboxFile> {
    try {
   ...
   */
  async getClientFolderStructure(clientFolderPath: string): Promise<{
    photos: string;
    documents: string;
    certificates: string;
    reports: string;
   ...
   */
  async searchFiles(query: string, folderPath?: string): Promise<DropboxFile[]> {
    try {
      ...
      for (const match of response.result.matches) {
        if (match.metadata.metadata['.tag'] === 'file') {
          const fileMetadata = match.metadata.metadata as any;
   ...
   */
  async syncClientFiles(clientFolderPath: string): Promise<{
    photos: DropboxFile[];
    documents: DropboxFile[];
    certificates: DropboxFile[];
    reports: DropboxFile[];
  }> {
    try {
...
export function createDropboxService(): DropboxService {
