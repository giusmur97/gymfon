...
import { createDropboxService } from './dropbox';
export class UnifiedCloudStorageService {
    constructor(primaryProvider = 'google_drive', backupProvider) {
     ...
     */
    async uploadFile(clientFolderId, options) {
        try {
            let primaryResult;
     ...
     */
    async getFileInfo(fileId, provider) {
     ...
     */
    async listClientFiles(clientFolderId, category) {
        try {
            if (this.primaryProvider === 'google_drive' && this.googleDrive) {
                const folderStructure = await this.googleDrive.getClientFolderStructure(clientFolderId);
     ...
     */
    async generateThumbnail(fileId, provider) {
        const targetProvider = provider || this.primaryProvider;
        try {
            if (targetProvider === 'google_drive' && this.googleDrive) {
     ...
     */
    async searchFiles(query, clientFolderId, provider) {
        const targetProvider = provider || this.primaryProvider;
        try {
            if (targetProvider === 'google_drive' && this.googleDrive) {
     ...
     */
    async syncFiles(clientFolderId) {
        if (!this.backupProvider || this.backupProvider === this.primaryProvider) {
        ...
        try {
            ...
            const categories = ['photos', 'documents', 'certificates', 'reports'];
    ...
    convertDriveFileToCloudFile(driveFile) {
        return {
            id: driveFile.id,
            name: driveFile.name,
    ...
    convertDropboxFileToCloudFile(dropboxFile) {
        return {
            id: dropboxFile.id,
...
export function createCloudStorageService(primaryProvider = 'google_drive', backupProvider) {
