...
export class NotificationService {
    ...
    async createInAppNotification(userId, title, message, type, data) {
        try {
    ...
    async sendEmail(to, template, fromName) {
        try {
            const mailOptions = {
    ...
    async sendSessionReminder(sessionId) {
        try {
            ...
            if (clientPrefs.sessionReminders.enabled) {
                const clientTemplate = this.generateSessionReminderTemplate(session.client.name, session.trainer.name, formattedDate, formattedTime, session.duration, session.location, session.meetingLink, 'client');
                // In-app notification
                if (clientPrefs.inApp) {
                    await this.createInAppNotification(session.clientId, 'Session Reminder', `Your training session with ${session.trainer.name} is starting soon`, 'session_reminder', { sessionId: session.id });
                }
                // Email notification
            ...
            if (trainerPrefs.sessionReminders.enabled) {
                const trainerTemplate = this.generateSessionReminderTemplate(session.trainer.name, session.client.name, formattedDate, formattedTime, session.duration, session.location, session.meetingLink, 'trainer');
                // In-app notification
                if (trainerPrefs.inApp) {
                    await this.createInAppNotification(session.trainerId, 'Session Reminder', `Your training session with ${session.client.name} is starting soon`, 'session_reminder', { sessionId: session.id });
                }
                // Email notification
    ...
    async sendSessionUpdateNotification(sessionId, updateType, changes) {
        try {
            const session = await prisma.trainingSession.findUnique({
                where: { id: sessionId },
                include: {
            ...
            if ((updateType === 'cancelled' && clientPrefs.sessionCancellations) ||
                ...
                if (clientPrefs.inApp) {
                    await this.createInAppNotification(session.clientId, title, `${message} with ${session.trainer.name} on ${formattedDate} at ${formattedTime}`, notificationType, { sessionId: session.id, changes });
                }
                if (clientPrefs.email) {
                    const template = this.generateSessionUpdateTemplate(session.client.name, session.trainer.name, formattedDate, formattedTime, updateType, session, changes);
            ...
            if (updateType === 'cancelled' && trainerPrefs.sessionCancellations) {
                if (trainerPrefs.inApp) {
                    await this.createInAppNotification(session.trainerId, title, `${message} with ${session.client.name} on ${formattedDate} at ${formattedTime}`, notificationType, { sessionId: session.id, changes });
                }
                if (trainerPrefs.email) {
                    const template = this.generateSessionUpdateTemplate(session.trainer.name, session.client.name, formattedDate, formattedTime, updateType, session, changes);
    ...
    generateSessionReminderTemplate(recipientName, otherPersonName, date, time, duration, location, meetingLink, recipientType = 'client') {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    ...
    generateSessionUpdateTemplate(recipientName, otherPersonName, date, time, updateType, session, changes) {
        let subject;
        let actionText;
        switch (updateType) {
            case 'created':
