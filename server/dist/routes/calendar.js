...
router.post('/sessions', authenticateToken, requireRole(['admin', 'staff']), async (req, res) => {
    try {
        const trainerId = req.user.id;
        const { clientId, type, date, duration, location, meetingLink, notes, price } = req.body;
...
router.put('/sessions/:id', authenticateToken, requireRole(['admin', 'staff']), async (req, res) => {
    try {
        const { id } = req.params;
        const trainerId = req.user.id;
        const { clientId, type, date, duration, location, meetingLink, notes, status, price } = req.body;
        ...
        for (const slot of availability) {
            ...
            while (currentTime.getTime() + sessionDuration * 60000 <= slotEnd.getTime()) {
                ...
                const hasConflict = existingSessions.some(session => {
                    ...
                    return ((currentTime >= sessionStart && currentTime < sessionEndTime) ||
